...
public class ValidationItemControllerV3 {
    ...
    ValidationItemControllerV3(ItemRepository itemRepository) {
     ...
     * - 예외가 발생하더라도 숫자 타입이므로 화면을 재로딩하면 문자는 사라져 사용자 본인이 어떤 내용을 입력했는지 모른다. (해결)
     * - 결국 고객이 입력한 값도 어딘가에 별도로 관리 되어야 한다. (해결)
     */
    @PostMapping("/add")
    public String addItem(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        if (bindingResult.hasErrors()) {
            ...
            return "validation/v3/addForm";     // addForm.html
        }
        // [E] 검증 로직
